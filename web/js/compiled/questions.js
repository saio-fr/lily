...
lily.ListQuestionsUnansweredView= Backbone.View.extend({/*A supprimer quand la fonction tri sera implÃ©mentÃ©e en back*/
    tagName: 'ul',
...
lily.ListQuestionsPersonalView= Backbone.View.extend({/*A supprimer quand la fonction tri sera implÃ©mentÃ©e en back*/
    tagName: 'ul',
	...
	displayArborescence: function() {
        ...
        if (versionDisplay.get('children')!== null) {
            if (versionDisplay.get('children').length == 0) {
                ...
                _.each(childrenToDisplay, function() {
                    if (childrenToDisplay[k].children.length == 0) {
					console.log('on affiche version rÃ©ponse');
                        arborescenceToDisplay.displayAnswer(childrenToDisplay[k]);
                    } else {
						console.log('on affiche version prÃ©cision');
                        arborescenceToDisplay.displayPrecision(childrenToDisplay[k]);
...
lily.QuestionPersonalFullView = Backbone.View.extend({
    ...
    save: function(e) {
        ...
        if (questionPersonal.get('answer') == null) {
            questionPersonal.set({
                answer: "Pas de rÃ©ponse enregitrÃ©e"
            });
            // alert('Attention, vous n\'avez pas rentrÃ© de rÃ©ponse pour cette question personnelle');
        }
		...
		*****************************/
		...
		_.each(app.listCategories,function(){
			if(app.listCategories.models[l].get('parent')!=null){
				...
				// app.listCategories.get(idParent).set({children:app.listCategories.models[l]});//on Ã©crase les enfants prÃ©cÃ©dents
			}
			l++;
		});	
		/*End of loop to create artificially categories children*/
		/*Bloc suivant Ã  supprimer quand problÃ¨me rÃ©glÃ© en back end*/
		_.each(app.listCategories,function(){
