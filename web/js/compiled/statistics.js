...
/*========================================
	  Chat Vue
	=========================================*/
	lily.ChatView = Backbone.View.extend({
		el: '#statistics-row',
		template: _.template($('#chat').html()),
		initialize: function() {
			this.render();
			this.graph = new lily.ChatGraphView();
		},
		render: function() {
			this.$el.html(this.template());
			return this;
...
/*======================================
	Graph Chat
	=========================================*/
	lily.ChatGraphView = Backbone.View.extend({
		el: '#graph',
		template: _.template($('#graph-chat').html()),
		templateFooter: _.template($('#graph-chat-footer').html()),
		events: {
			'click .numberOfConversation': 'numberOfConversation',
			'click .conversationTime': 'conversationTime',
			'click .waited': 'waited',
		...
		render: function() {
			this.footer = new lily.Data();
			this.footer.url = '/chat/graph/footer/' + this.start + '/' + this.end;
			that = this;
			this.footer.fetch({
				success: function() {
					that.$el.html(that.template());
		...
		numberOfConversation: function() {
			...
			$('.loader').removeClass('hide');
			data = new lily.Data();
			data.url = '/chat/graph/conversations/' + this.start + '/' + this.end;
			$(this.$el).find('#graph-el').animate({
				opacity: 0
			});
			$('.loader').fadeIn();
			var that = this;
			data.fetch({
				success: function(data) {
					...
					$('.numberOfConversation').addClass('active');
					that.graph(data);
					$(that.$el).find('#graph-chat-stats').animate({
						opacity: 1
					});
					$(that.$el).find('footer').animate({
						opacity: 1
		...
		conversationTime: function() {
			data = new lily.Data();
			data.url = '/chat/graph/conversationsTime/' + this.start + '/' + this.end;
			$(this.$el).find('#graph-chat-stats').animate({
				opacity: 0
			});
			$('.loader').fadeIn();
			var that = this;
		...
		waited: function() {
			data = new lily.Data();
			data.url = '/chat/graph/waited/' + this.start + '/' + this.end;
			$(this.$el).find('#graph-chat-stats').animate({
				opacity: 0
			});
			$('.loader').fadeIn();
		...
		satisfaction: function() {
			data = new lily.Data();
			data.url = '/chat/graph/satisfaction/' + this.start + '/' + this.end;
			$(this.$el).find('#graph-chat-stats').animate({
				opacity: 0
			});
			$('.loader').fadeIn();
