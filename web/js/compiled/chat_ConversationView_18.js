...
chat.Views.Conversation = Backbone.View.extend({
	...
	addItem: function( message ) {
		...
		this.operators = this.messages.where({ from: 'operator' });
		this.index = this.operators.indexOf(message);
		
		if (this.index >= 1) {
			// If the operator changed
					console.log(this.operators[this.index-1]);
			if ( message.get('operator') !== this.operators[this.index-1].get('operator') ) {
				
				this.operator = new chat.Model.Operator();
				this.operator.url = '/users/rest/'+message.get('operator');
				this.operator.fetch();
				this.operator.set({'date': message.get('date')});
				
				this.messagesHeader = new chat.Views.MessagesHeader({ model: this.operator });
				this.messagesHeader.$el.appendTo( '.conversation-section-list' );
			...
			this.operator.url = '/users/rest/'+message.get('operator');
			this.operator.fetch();
			this.operator.set({'date': this.message.get('date')});
