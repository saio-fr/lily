...
lily.Models.MessageUserSimple=Backbone.Model.extend({defaults:{message_content:""}});
...
lily.Views.MessageView=Backbone.View.extend({initialize:function(){this.model.on("change",this.render,this)},render:function(){this.$el.html(this.template(this.model.toJSON()));this.$el.appendTo("#lily-box-messages");this.transitionInMessage();this.trigger("render");return this},transitionInMessage:function(callback){var $message=this.$el;var inClass="lily-message-show";$message.addClass(inClass).on(animEndEventName,function(){$message.off(animEndEventName);$("#lily-box-messages").animate({scrollTop:$("#lily-box-messages").get(0).scrollHeight},500);console.log("t")})}});
...
lily.Views.MessageChatOperator=lily.Views.MessageChatView.extend({className:"lily-msg-chat-operator lily-cst-msg-chat-operator",model:lily.Models.MessageLilySimple,template:_.template($("#chat-message-operator").html())});
...
lily.Views.MessageLilyPrecision=lily.Views.MessageView.extend({className:"lily-msg-precision lily-cst-msg-precision",model:lily.Models.MessageLilyPrecision,template:_.template($("#lily-message-precision").html()),initialize:function(){this.listenTo(this,"render",this.triggerPrecision)},triggerPrecision:function(){var message=this;this.$(".lily-precision-list").on("click",function(){var list_index=message.$("li.lily-precision-list").index($(this));var id=message.model.get("actions")[list_index].id;var idparent=message.model.get("idparent");lily.Events.trigger("precision",id,idparent,message);$(this).off("click")})}});
lily.Views.MessageLilyRedirection=lily.Views.MessageView.extend({className:"lily-msg-redirection lily-cst-msg-redirection",template:_.template($("#lily-message-redirection").html()),initialize:function(){this.listenTo(this,"render",this.triggerRedirectionTel);this.listenTo(this,"render",this.triggerRedirectionMail)},redirectionMail:function(){$.ajax({type:"POST",url:"http://saio.fr/web/app_dev.php/api/"+key+"/redirection/"+this.model.get("data").id+"/mail",data:"mail",success:function(data){console.log("Log sent")}})},triggerRedirectionTel:function(){var view=this;this.$(".lily-redirection-tel").on("click",function(){var id=view.model.get("data").id;lily.Events.trigger("redirectionTel",id,"true","",view);$(this).off("click")})},triggerRedirectionMail:function(){var view=this;this.$(".lily-redirection-mail").on("click",function(){var id=view.model.get("data").id;lily.Events.trigger("redirectionMail",id,"true","",view);$(this).off("click")})}});
lily.Views.MessageLilyNotation=Backbone.View.extend({className:"lily-msg-reporting lily-cst-msg-reporting",model:lily.Models.MessageLilyNotation,template:_.template($("#lily-message-notation").html()),events:{"click .lily-icon-thumb-up":"satisfied","click .lily-icon-thumb-down":"notSatisfied"},initialize:function(){this.render()},render:function(){this.$el.html(this.template(this.model.toJSON()));this.$el.appendTo("#lily-box-messages .lily-msg-avatar:last");$("#lily-box-messages .lily-msg-avatar:last").addClass("lily-notation-wrapper");return this},satisfied:function(){lily.Events.trigger("satisfied",this.model.get("id"),true,"",this)},notSatisfied:function(){lily.Events.trigger("notSatisfied",this.model.get("id"),false,"",this)}});
lily.Views.MessageLilyCompletion=lily.Views.MessageView.extend({className:"lily-msg-reporting lily-cst-msg-reporting",model:lily.Models.MessageLilyCompletion,template:_.template($("#lily-message-completion").html()),initialize:function(){this.listenTo(this,"render",this.triggerCompletion)},triggerCompletion:function(){var view=this;this.$(".lily-precision-list").on("click",function(){var id=view.model.get("id");if($(this).hasClass("lily-completion-incomplete")){lily.Events.trigger("satisfied",id,"false","incomplete",view)}if($(this).hasClass("lily-completion-incorrect")){lily.Events.trigger("satisfied",id,"false","incorrect",view)}else return false;$(this).off("click")})}});
lily.Views.Faq=lily.Extensions.View.extend({model:lily.Models.Faq,template:_.template($("#lily-page-faq-template").html()),initialize:function(){$(this.render().el).appendTo("#lily-wrapper-page")},render:function(){this.$el.html(this.template(this.model.toJSON()));return lily.Extensions.View.prototype.render.apply(this,arguments)}});
...
lily.Views.Content=lily.Extensions.View.extend({model:lily.Models.Content,template:_.template($("#lily-page-content-template").html()),initialize:function(){$(this.render().el).appendTo("#lily-wrapper-page")},render:function(){this.$el.html(this.template(this.model.toJSON()));return lily.Extensions.View.prototype.render.apply(this,arguments)}});
lily.Views.Home=lily.Extensions.View.extend({mood:"",messageModel:"",messageType:"",events:{"submit #lily-search-form":"doSearch","click #lily-go":"doSearch"},initialize:function(){this.$input=this.$("#lily-search-form input.lily-search-input");this.listenTo(this,"render",this.avatar);lily.Events.on("precision",this.sendPrecision,this);lily.Events.on("satisfied",this.sendNotation,this);lily.Events.on("notSatisfied",this.sendNotation,this);lily.Events.on("redirectionTel",this.sendRedirectionTel,this);lily.Events.on("redirectionMail",this.sendRedirectionMail,this);$(this.render().el).appendTo("#lily-wrapper-page")},render:function(){var template=_.template($("#lily-page-home-template").html());this.$el.html(template());this.trigger("render");$("input, textarea").placeholder();return lily.Extensions.View.prototype.render.apply(this,arguments)},doSearch:function(e){e.preventDefault();this.$input=this.$el.find("#lily-search-form input.lily-search-input");var query=this.$input.val();if($.trim(query).length>0){this.search(query);var messageModel=new lily.Models.MessageUserSimple({message_content:this.$input.val()});this.addItem(messageModel,"user-simple");this.showLoading()}this.clearInput()},search:function(q){var home=this;$.ajax({type:"POST",url:root+"/query",data:{query:q},success:function(data,textStatus,request){var type=request.getResponseHeader("type");home.clearLoading();if(!data){this.trigger("emptySearch")}if(type==="insult"){var messageModel=new lily.Models.MessageLilySimple({message_content:data});var messageType="lily-simple";home.mood="angry";home.addItem(messageModel,messageType)}if(type==="answer"){var messageModel=new lily.Models.MessageLilySimple({message_content:data.answer});var messageType="lily-simple";home.addItem(messageModel,messageType);var messageModel=new lily.Models.MessageLilyNotation({id:data.id});var messageType="lily-notation";home.mood=data.mood;home.addItem(messageModel,messageType)}if(type==="personal"){var messageModel=new lily.Models.MessageLilySimple({message_content:data.answer});var messageType="lily-simple";home.mood=data.mood;home.addItem(messageModel,messageType)}if(type==="misunderstood"){var messageModel=new lily.Models.MessageLilySimple({message_content:"Désolé, je n'ai pas compris votre question."});var messageType="lily-simple";home.addItem(messageModel,messageType);if(data.isMail||data.isTel||data.isChat){var messageModel=new lily.Models.MessageLilyRedirection({data:data});var messageType="lily-redirection";home.mood="sceptical";home.addItem(messageModel,messageType)}}if(type==="precision"){var messageModel=new lily.Models.MessageLilyPrecision({actions:data.actions,precision:data.parent.answer,idparent:data.parent.id});var messageType="lily-precision";home.mood=data.mood;home.addItem(messageModel,messageType)}}})},sendPrecision:function(id,idparent,parent){var home=this;$.ajax({url:root+"/precision/"+id,data:id,success:function(data,textStatus,request){console.log(data);var type=request.getResponseHeader("type");console.log(type);home.clearLoading();if(!data){this.trigger("emptySearch");home.clearLoading()}if(type==="answer"){var messageModel=new lily.Models.MessageLilySimple({message_content:data.answer});var messageType="lily-simple";home.addItem(messageModel,messageType);var messageModel=new lily.Models.MessageLilyNotation({id:idparent});var messageType="lily-notation";home.addItem(messageModel,messageType)}}})},sendRedirectionMail:function(id){var id=id;$.ajax({type:"POST",url:root+"/logredirection/"+id,data:JSON.stringify({canal:"mail"}),success:function(data){console.log("Log sent")}})},sendRedirectionTel:function(id){var id=id;$.ajax({type:"POST",url:root+"/logredirection/"+id,data:JSON.stringify({canal:"tel"}),success:function(data){console.log("Log sent")}})},sendNotation:function(id,satisfied,reason,view){var home=this;var id=id;$.ajax({type:"POST",url:root+"/notation/"+id,data:JSON.stringify({satisfied:satisfied,reason:reason}),success:function(data,textStatus,request){if(satisfied==true){var messageModel=new lily.Models.MessageLilySimple({message_content:"Merci pour votre réponse! N'hesitez pas à me poser d'autres questions"});var messageType="lily-simple"}else{var messageModel=new lily.Models.MessageLilyRedirection({data:data});var messageType="lily-redirection"}home.addItem(messageModel,messageType);view.remove()}})},addItem:function(messageModel,messageType){switch(messageType){case"user-simple":var message=new lily.Views.MessageUserSimple({model:messageModel}).render();break;case"lily-simple":var message=new lily.Views.MessageLilySimple({model:messageModel}).render();break;case"lily-redirection":var message=new lily.Views.MessageLilyRedirection({model:messageModel}).render();break;case"lily-precision":var message=new lily.Views.MessageLilyPrecision({model:messageModel}).render();break;case"lily-notation":var message=new lily.Views.MessageLilyNotation({model:messageModel}).render();break;case"lily-completion":var message=new lily.Views.MessageLilyCompletion({model:messageModel}).render();break}},showLoading:function(){this.$("#lily-box-messages").append('<div class="lily-msg-avatar lily-cst-msg-avatar lily-message-show"><p class="lily-loading"><span></span><span></span><span></span></p><i class="lily-avatar-bubble"></i></div>')},clearInput:function(){if(isMobile.phone){this.$input.val("").blur()}else this.$input.val("")},clearLoading:function(){if(this.$(".lily-loading").length){setTimeout(function(){this.$(".lily-loading").parent().fadeOut(function(){$(this).remove()})},500)}},emptySearch:function(){this.clearLoading();var model=new lily.Models.MessageLilySimple({message_content:"Il semblerait qu'il y ait un problème. Je n'ai pas trouvé de réponse à votre question"});this.message=new lily.Views.MessageLilySimple({model:model}).render()},avatar:function(){home=this;setTimeout(function(){$.getScript(root+"/avatar",function(data){})},500)}});
lily.Views.Chat=lily.Extensions.View.extend({events:{"keyup #lily-search-form":"writing","submit #lily-search-form":"doChat","click #lily-go":"doChat"},initialize:function(){chat=this;this.messages=new lily.Messages;this.listenTo(this.messages,"add",this.addItem);sess=new ab.connect("ws://dev2.saio.fr:8080/chat/"+licence,function(session){sid=document.cookie.match("PHPSESSID=([^;]*)")[1];sess=session;sess.subscribe("visitor/"+sid,function(topic,payload){chat.messages.set(payload);sess.call("chat/writing",{sid:sid,writing:false})})},function(code,reason,detail){console.warn("WebSocket connection closed")},{skipSubprotocolCheck:true,maxRetries:60,retryDelay:2e3});$(this.render().el).appendTo("#lily-wrapper-page")},render:function(){var template=_.template($("#lily-page-chat-template").html());this.$el.html(template());this.trigger("render");$("input, textarea").placeholder();this.$input=this.$el.find("#lily-search-form input.lily-search-input");return lily.Extensions.View.prototype.render.apply(this,arguments)},writing:function(e){if(this.$input.val()){sess.call("chat/writing",{sid:sid,writing:true})}else{sess.call("chat/writing",{sid:sid,writing:false})}},doChat:function(e){e.preventDefault();var message=this.$input.val();if($.trim(message).length>0){this.send(message)}this.clearInput()},send:function(m){sess.publish("operator",m)},addItem:function(message){switch(message.get("from")){case"visitor":var messageView=new lily.Views.MessageChatVisitor({model:message}).render();break;case"operator":var messageView=new lily.Views.MessageChatOperator({model:message}).render();break}},clearInput:function(){if(isMobile.phone){this.$input.val("").blur()}else{this.$input.val("").focus().select()}},avatar:function(){return this}});
...
var lily=lily||{};lily.Router=Backbone.Router.extend({url:"",routes:{"top-questions":"topQuestions","top-questions/":"topQuestions","top-questions/:id":"topQuestions",faq:"faq","faq/":"faq","faq/:id":"faq","faq/content/:id":"content","":"home",home:"home",chat:"chat",mail:"mail","mail/sent":"mailSent"},home:function(){if(typeof view!=="undefined"){view.remove()}var view=new lily.Views.Home;lily.instance.goTo(view);this.welcome=new lily.Models.MessageLilySimple({message_content:aviWelcomeMsg});this.message=new lily.Views.MessageLilySimple({model:this.welcome}).render()},mailSent:function(){if(typeof view!=="undefined"){view.remove()}var view=new lily.Views.Home;lily.instance.goTo(view);this.welcome=new lily.Models.MessageLilySimple({message_content:"Votre message a bien été envoyé. Que puis-je faire pour vous ?"});this.message=new lily.Views.MessageLilySimple({model:this.welcome}).render()},chat:function(){if(typeof view!=="undefined"){view.remove()}var view=new lily.Views.Chat;lily.instance.goTo(view)},mail:function(){if(typeof view!=="undefined"){view.remove()}var view=new lily.Views.Mail;lily.instance.goTo(view)},faq:function(id){if(typeof view!=="undefined"){view.remove()}var home=this;if(id==""||id==undefined||id=="/")id="NULL";$.ajax({url:root+"/faq/"+id,success:function(data,textStatus,request){if(!data){}if(data!==""){_.each(data.faqs,function(faq){if(faq.type==="content"){var id=faq.id;faq.id="content/"+id}});var sortedData=_.sortBy(data.faqs,data.faqs.position);var model=new lily.Models.Faq({data:{parent:data.parent,title:data.title,faqs:sortedData}});var view=new lily.Views.Faq({model:model});lily.instance.goTo(view)}}})},content:function(id){var id=id;if(typeof view!=="undefined"){view.remove()}var home=this;$.ajax({url:root+"/faq/"+id,success:function(data,textStatus,request){console.log(data);if(!data){}if(data!==""){var idParent=data.parent;$.ajax({url:root+"/faq/"+idParent,success:function(data,textStatus,request){if(!data){}if(data!==""){var content=_.find(data.faqs,function(obj){return obj.id==id}).content;var title=_.find(data.faqs,function(obj){return obj.id==id}).title;var model=new lily.Models.Content({parent:idParent,title:title,content:content});var view=new lily.Views.Content({model:model});lily.instance.goTo(view)}}})}}})},topQuestions:function(id){if(typeof view!=="undefined"){view.remove()}var home=this;if(id==""||id==undefined||id=="/"){$.ajax({url:root+"/top-questions/NULL",success:function(data,textStatus,request){if(!data){}if(data!==""){var model=new lily.Models.TopQuestions({data:data});var view=new lily.Views.TopQuestions({model:model});lily.instance.goTo(view)}}})}else{$.ajax({url:root+"/top-questions/"+id,success:function(data,textStatus,request){if(!data){}if(data!==""){var view=new lily.Views.Home;lily.instance.goTo(view);var question=new lily.Models.MessageUserSimple({message_content:data.title});var reponse=new lily.Models.MessageLilySimple({message_content:data.answer});view.addItem(question,"user-simple");view.addItem(reponse,"lily-simple")}}})}}});
=======
