...
//Fonctions
	...
	function breathe () {
		...
		//Breasts animation
		var breasts_lefty = breasts_lefty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));
		breasts_left.position.y = breasts_lefty;

		var breasts_righty = breasts_righty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));
		breasts_right.position.y = breasts_righty;

		//Shoulders animation
		var collar_backy = collar_backy_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		collar_back.position.y = collar_backy;

		var polo_toprighty = polo_toprighty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		polo_topright.point.y = polo_toprighty;

		var polo_toplefty = polo_toplefty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		polo_topleft.point.y = polo_toplefty;

		var chesty = chesty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		chest.position.y = chesty;

		var chest_shadow_lefty = chest_shadow_lefty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		chest_shadow_left.position.y = chest_shadow_lefty;

		var chest_shadow_righty = chest_shadow_righty_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		chest_shadow_right.position.y = chest_shadow_righty;

		var polo_shadowy = polo_shadowy_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		polo_shadow.point.y = polo_shadowy;

		var neck_shadowy = neck_shadowy_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		neck_shadow.position.y = neck_shadowy;

		var collar_lefty = collar_left_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		collar_left.position.y = collar_lefty;

		var collar_righty = collar_right_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
		collar_right.position.y = collar_righty;

		var collar_shadowy = collar_shadow_init - 2 * Math.abs(Math.sin(time * Math.PI /freq_breathe));;
	...
	function toAngry () {
		...
		var eyebrow_leftx = eyebrow_left.position.x + 0.1 * Math.abs(Math.sin(time * Math.PI /2*freq));
		eyebrow_left.position.x = eyebrow_leftx;

		var eyebrow_rightx = eyebrow_right.position.x - 0.1 * Math.abs(Math.sin(time * Math.PI /2*freq));
