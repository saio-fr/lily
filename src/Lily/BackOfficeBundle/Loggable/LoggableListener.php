...
namespace Lily\BackOfficeBundle\Loggable;
...
class LoggableListener extends GedmoLoggableListener
{
    ...
    protected function createLogEntry($action, $object, LoggableAdapter $ea)
    {
        ...
        if ($config = $this->getConfiguration($om, $meta->name)) {
			...
			// OVERRIDE
			$entity = explode('\\', $meta->name);
			$entity = strtolower($entity[3]);
			
			$title = $object->getTitle();
			
			if ($meta->name == "Lily\BackOfficeBundle\Entity\Question") {
			
			  if ($object->getParent() !== NULL) {
			    return false;
			  }
			  			
			  $satisfaction = $object->getSatisfaction();
			  $logEntry->setSatisfaction($satisfaction);			  
			  
			} else {
			  $logEntry->setSatisfaction(0);
			}
